        ...
        for filename, data in target_slice.items():
            if "iv_data_rbias" in data:
                ...
                if len(rbias_data) > 20:
                    # print(f"{filename} has more than 40 rbias measurements, checking for outliers...")
                    #cleaned_t_s[filename] = data  
                    use = True  
                    for x, y in rbias_data:
                        if 10<= x <= 30 and abs(y) > 1e-8:
                            use = False
                        else:
                            pass
                    if use == True:
...
class Graph():
    ...
    def save(self, *, folder: typing.Optional[str] = None, to_stdout: bool = False) -> None:
