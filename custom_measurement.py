...
def do_IV(*, dev, graph = [], start, stop, step, delay, is_last=False, logger):
    try:
...
def measure(*, msgServer, device_port, folder, diode_Nr, settings, logger, plotting=False, extra, tag_1, tag_2, tag_3, measurements_per_diode, measurement_no):
    try:
        ...
        iv_data_rbias = []
        iv_data_fbias = []
        iv_data_rbias = do_IV(dev=device_port, graph=iv_data_rbias, start=start_rbias, stop=stop_rbias, step=step_rbias, delay=delay_rbias, is_last=False, logger=logger)
        iv_data_fbias = do_IV(dev=device_port, graph=iv_data_fbias, start=start_fbias, stop=stop_fbias, step=step_fbias, delay=delay_fbias, is_last=True, logger=logger)
        ...
        if iv_data_rbias and iv_data_fbias:
