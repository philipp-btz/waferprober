...
def get_ch_in_sim(matrix,LocID,opt=1): #opt=1l return channel in Simulation; opt=2: return x,y coordination in siulation
    ...
    if opt==1:
        ...
        return -1
def getAmplitude_along_HV(file_name,dir_out=None):
    file_tmp = ROOT.TFile(file_name)
    gr_Amp = ROOT.TGraphErrors()
    gr_StdDev = ROOT.TGraph()
    for hv in range(54000,56000,200):
        h_amp = ROOT.TH1D(f"Amplitude_HV{hv}mV",f"Amplitude distibution; Amplitude [mV];Entries ",1000,-10.,10.)
        print("hv: ",hv)
        ...
        n=gr_tmp.GetN()
        #x=gr_tmp.GetX()
        y=np.asarray(gr_tmp.GetY())
        #for index in range(gr_tmp.GetN()):
        #    if(index%1000==0) 
        #    h_amp.Fill(gr_tmp.GetY()[index])
        
        h_amp.FillN(n,y,0)
        if dir_out!=None:
            dir_out.cd()
            h_amp.Write()
        mean_tmp = h_amp.GetMean()
        stddev_tmp = h_amp.GetStdDev()
        
        hv_corr = mean_tmp/50.*1000.
        print("mean_tmp, stddev: ",mean_tmp,stddev_tmp)
        gr_Amp.SetPoint(gr_Amp.GetN(),(hv-hv_corr)/1000.,mean_tmp)
        gr_Amp.SetPointError(gr_Amp.GetN()-1,0.,stddev_tmp)
        gr_StdDev.SetPoint(gr_StdDev.GetN(),(hv-hv_corr)/1000.,stddev_tmp)
        #free(gr_tmp)
    file_tmp.Close() 
    gr_Amp.SetNameTitle(f"Amplitude_matrix{iMatrixID}_ID{LocID}",f"Amplitude of Signal (matrix{iMatrixID}, CH{LocID}); HV [V]; Amplitude [mV]")
    gr_StdDev.SetNameTitle(f"StdDev_matrix{iMatrixID}_ID{LocID}",f"StdDev of Signal (matrix{iMatrixID}, CH{LocID}); HV [V]; Amplitude [mV]")
    return gr_Amp,gr_StdDev 
#==============================
#==============================
if(len(sys.argv)!=5 ):
...
dir_AmpGr = file_out.mkdir("AmpPlot/")
dir_AmpHist = file_out.mkdir("AmpHist/")
...
for LocID in [ch_interested]:
    ...
    for iMatrixID in [matrix_interested]:
        ...
        if not Path(file_name).is_file():
            ...
            continue
        gr_Amp,gr_StdDev=getAmplitude_along_HV(file_name=file_name,dir_out = dir_AmpHist)
        dir_AmpGr.cd()
        gr_Amp.Write()
        gr_StdDev.Write()
        grs_corr.append(gr_Amp)#TODO check TGraph exist or not

        
        for index in range(5): 
            grs_HVconst_UIDmapping[index].SetPoint(grs_HVconst_UIDmapping[index].GetN(),iMatrixID*20+ch_to_UID[LocID],gr_Amp.Eval(index+52))
            if iMatrixID<7 and iMatrixID>0:#mapping to simulation channel
                grs_HVconst[index].SetPoint(grs_HVconst[index].GetN(),get_ch_in_sim(iMatrixID,LocID),gr_Amp.Eval(index+52))
                x,y =get_ch_in_sim(iMatrixID,LocID,2)
                hs_HVconst[index].Fill(8-(x+0.5),y+0.5,gr_Amp.Eval(index+52))
...
for index in range(len(grs_HVconst)):
    grs_HVconst[index].SetNameTitle(f"gr_Current_HV{52+index}",f"Dark Current HV={52+index}V; Ch ID in simulation; Amplitude [mV]")
    grs_HVconst_UIDmapping[index].SetNameTitle(f"gr_Current_HV{52+index}_IV_mapping",f"Dark Current HV={52+index}V; UID on PCB; Ampulitude [mV]")
    hs_HVconst[index].SetNameTitle(f"h2d_Current_HV{52+index}",f"Dark Current HV={52+index}V; Ch ID along x; Ch ID along y; Amplitude [mV]")
